...
import br.com.facility.service.PublicacaoService;
...
public class PublicacaoResource {
	...
	public ResponseEntity<List<PublicacaoDTO>> findAll() {
		...
		if (publicacoes == null || publicacoes.isEmpty()) {
			return new ResponseEntity<List<PublicacaoDTO>>(HttpStatus.NO_CONTENT);
		}
		return new ResponseEntity<List<PublicacaoDTO>>(publicacoes, HttpStatus.OK);
	}
	...
	public ResponseEntity<?> findById(@PathVariable Long id) {
		return publicacaoService.findById(id).map(record -> ResponseEntity.ok().body(record))
				.orElse(ResponseEntity.notFound().build());
	}

	@PutMapping(value = "/{id}")
	public ResponseEntity<Publicacao> update(@PathVariable("id") Long id, @RequestBody Publicacao publicacao) {
		return publicacaoService.findById(id).map(record -> {
